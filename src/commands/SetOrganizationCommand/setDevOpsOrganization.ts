...
export const setDevOpsOrganization = () => {
	const commandHandler = async () => {
		...
		const prevOrganization = config.get<string>("devopsOrganization", "");
		const organization = await window.showInputBox({ title: "Input your DevOps organization", value: prevOrganization });
		if (!organization) { throw new OrganizationNotProvidedError(); };

		await config.update("devopsOrganization", organization);
