...
export const getTeamContext = (): TeamContext => ({ team: config.getProjectKey("devopsTeam"), project: config.getProjectKey("devopsProject") });
export const getApi = (instance: string = DEFAULT_INSTANCE_URL): WebApi => {
	...
	return new WebApi(`${customInstance ? (instance !== DEFAULT_INSTANCE_URL ? instance : customInstance) : instance}/${organization}`, authHandler);
};

async function _profile(id: string = "me") { return (await (getApi(VSSPS_INSTANCE).getProfileApi())).getProfile(id) as Promise<Profile & { displayName: string, emailAddress: string }>; };
