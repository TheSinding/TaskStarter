...
const getProjectObject = (projectName: string) => {
    ...
    if (!projectConfig) { return {} as ProjectConfig; };
...
export const getProjectKey = <K extends keyof ProjectConfig>(section: K, defaultValue?: Required<ProjectConfig>[K]): ProjectConfig[K] => {
    const projectName = getProjectName();
    if (!projectName) { throw new Error("No project found"); }
